...
public class Othello extends Application{
    private static final int WIDTH = 320;//盤面の横幅
    private static final int HEIGHT = 320;//盤面の高さ
    private static final int GRID_X = 8;//マスの横幅
    public static final int GRID_Y = 8;//マスの高さ
    ...
    public void start (Stage stage)throws Exception{
        ...
        for(int loopY = 0; loopY < GRID_Y; loopY++){
            g.strokeLine(0, loopY * GRID_SIZE, WIDTH, loopY * GRID_SIZE);
        }

         //盤面の縦線
        for(int loopX = 0; loopX < GRID_X; loopX++){
            g.strokeLine(loopX * GRID_SIZE, 0, loopX *GRID_SIZE, HEIGHT);
        }

        //初期配置の石を表示
        for(int loopY =0; loopY < GRID_Y; loopY++){
            for(int loopX = 0; loopX < GRID_X; loopX++){
                g.setFill(gi.getGridStoneColor(loopX, loopY));
    ...
    public void mouseClicked(MouseEvent e){
        ...
        //クリックしたマスに手番が石を置けるか判定し、可能なら手番の色の石をセットする
        gi.reverseGridStateFlg(clickTileX, clickTileY);
        for(int loopY = 0; loopY < GRID_Y; loopY++){
            for(int loopX = 0; loopX < GRID_X; loopX++){
                g.setFill(gi.getGridStoneColor(loopX, loopY));
                g.fillOval(loopX * GRID_SIZE + 5, loopY * GRID_SIZE + 5, 30, 30);
        ...
        //手番が石を置けるマスにグレー丸を表示する
        for(int loopY = 0; loopY < GRID_Y; loopY++){
            for(int loopX = 0; loopX < GRID_X; loopX++){
                gi.resetGrayStone(loopX, loopY);
                gi.chkCanSetStone(loopX, loopY);
                g.setFill(gi.getGrayStoneColor(loopX, loopY));
                g.fillOval(loopX * GRID_SIZE + 12.5, loopY * GRID_SIZE + 12.5, 15, 15);
            }
        }

        //手番の表示を交代させる
        tebanFlg = gi.getTebanFlg();
        if(tebanFlg == gi.BLACK_STATE){
            text.setText("●黒の番です");
        }else{
            text.setText("○白の番です");
        }
...
class GridInfo{
    ...
    //手番が石を置ける位置の色を指定するメソッド
    public Color getGrayStoneColor(int x, int y){
        if(gridStateFlgGray[y][x] == GRAY_STATE){
            return Color.GRAY;
        }else if(gridStateFlg[y][x] == BLACK_STATE){
            return Color.BLACK;
        }else if(gridStateFlg[y][x] == WHITE_STATE){
            return Color.WHITE;
        }else{
            return Color.GREEN;
        }
